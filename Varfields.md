...
| 34 | `VarField_iremiss` | `IREmiss(:)` | surface emissivity (satellite 1dvar) | From a variable generated by the 1D-Var filter or variable assignment | 'ir_emissivity@Emiss' |
| 35 | `VarField_cloudtopp` | `CloudTopP(:)` | cloud top pressure (hPa) (satellite 1dvar) | `OneDVar/cloud_top_pressure` | |
| 36 | `VarField_cloudfrac` | `CloudFrac(:)` | cloud fraction (satellite 1dvar) | `OneDVar/cloud_fraction` | |
...
| 57 | `VarField_Emissivity` | `Emissivity(:,:)` | surface emissivity (satellite 1dvar) | OneDVar/surface_emissivity | |
...
| 76 | `VarField_BriTempVarError` | `BriTempVarError(:,:)` | scene dependent obs errors for BTs (oblocation, channel) | `brightness_temperature@ObsErrorData` | |
| 77 | `VarField_AOD` | `AOD(:,:)` | aerosol optical depth | `aerosolOpticalDepth@ObsValue` | |
