  ...
  select case (VarFields(iVarField))
    ...
    case (VarField_britemp)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "brightnessTemperature", "BiasCorrObsValue", &
	self % channel_offset, self % useActualChannels)
    case (VarField_tskin)
    ...
    case (VarField_mwemiss)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "emissivity", "Emiss")
    case (VarField_TCozone)
      call opsinputs_fill_fillreal( &
    ...
    case (VarField_Emissivity)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "emissivity", "OneDVar", self % channel_offset)
    ...
    case (VarField_RadarObAzim)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "radarAzimuth", "MetaData")
    ...
    case (VarField_BriTempVarError)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "brightnessTemperature",&
        "ObsErrorData", self % channel_offset)
    ...
    case (VarField_LevelLat)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "latitude", "MetaData")
    case (VarField_LevelLon)
      call opsinputs_fill_fillreal2d( &
        Ob % Header % level_lon, "level_lon", JediToOpsLayoutMapping, Ob % level_lon, &
        ObsSpace, self % jopaChannels, self % varChannels, self % VarobsLength, "longitude", "MetaData")
...
subroutine opsinputs_varobswriter_fillchannumandnumchans( &
  Ob, ObsSpace, jopaChannels, varChannels, Flags, FillChanNum, FillNumChans, &
...
integer(integer64)             :: ChannelIndices(Ob % Header % NumObsLocal, size(jopaChannels))
...
! Body:
NumChannels = size(jopaChannels)
if (NumChannels == 0) return

call opsinputs_varobswriter_findchannelspassingqc( &
  Ob % Header % NumObsLocal, ObsSpace, jopaChannels, Flags, ChannelIndices, ChannelCounts)
  
if (FillChanNum) then
  call Ops_Alloc(Ob % Header % ChanNum, "ChanNum", Ob % Header % NumObsLocal, Ob % ChanNum, &
                 num_levels = NumChannels)
  if (size(jopaChannels) == size(varChannels)) then
