...
do iVarField = 1, nVarFields
  select case (VarFields(iVarField))
    ...
    case (VarField_britemp)
      ...
      call opsinputs_fill_fillreal2d( &
        ...
        self % size_of_varobs_array, self % varChannels)
    case (VarField_tskin)
    ...
    case (VarField_mwemiss)
      ...
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % channels, self % VarobsLength, "emissivity", "Emiss", self % size_of_varobs_array, self % varChannels)
    case (VarField_TCozone)
      call opsinputs_fill_fillreal( &
    ...
    case (VarField_Emissivity)
      call opsinputs_fill_fillreal2d( &
        ...
        ObsSpace, self % channels, self % VarobsLength, "emissivity", "OneDVar", self % size_of_varobs_array, &
        self % varChannels)
    ...
    case (VarField_BriTempVarError)
      call opsinputs_fill_fillreal2d( &
        ...
        "ObsErrorData", self % size_of_varobs_array, self % varChannels)
  ...
  if (FillChanNum .or. FillNumChans) then
    ...
    call opsinputs_varobswriter_fillchannumandnumchans(  &
      ...
      FillNumChans, self % size_of_varobs_array)
  end if
...
subroutine opsinputs_varobswriter_fillchannumandnumchans( &
  Ob, ObsSpace, channels, varChannels, Flags, FillChanNum, FillNumChans, varObsSize)
...
integer(integer64), intent(in) :: varObsSize
    ...
    if (size(channels) == size(varChannels)) then
      ...
      !  end if
        ...
        do iObs=1, Ob % Header % NumObsLocal
          ...
          WRITE(*,*) "ChannelIndicesVar3=", ChannelIndicesVar(:,3)
        end do
      end do
