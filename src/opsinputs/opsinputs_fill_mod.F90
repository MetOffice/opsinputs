...
!> is included in the list passed to the 'simulate' option of the JEDI ObsSpace.
!>
!> This is the implementation used for when each OPS observation should correspond to a single JEDI
!> location, with multiple channels for the obsvalues, but only a single dimension for obserrors, flags and PGE.
!>
!> \param[inout] Hdr
!>   Header to be populated.
!> \param[in] OpsVarName
...
!> We rely on warnings printed by the OPS code whenever data needed to output a requested varfield
!> are not found.
subroutine opsinputs_fill_fillelementtypefromsimulatedvariable2dobsvalue( &
  Hdr, OpsVarName, NumObs, El2, ObsSpace, Channels, Flags, ObsErrors, JediVarName, PackPGEs)
implicit none
...
integer                                         :: iChannel, iObs
character(len=*), parameter                     :: &
  RoutineName = "opsinputs_fill_fillelementtypefromsimulatedvariable2dobsvalue"
character(len=256)                              :: ErrorMessage
...
if (obsspace_has(ObsSpace, "ObsValue", JediVarNamesWithChannels(1))) then
  ...
  do iChannel = 1, size(JediVarNamesWithChannels)
    ...
    if (opsinputs_obsdatavector_int_has(Flags, JediVarName)) then
      ...
      write (ErrorMessage, '(A,A,A)') &
        "Warning: variable ", JediVarName, " QC flag not found"
      call gen_warn(RoutineName, ErrorMessage)
      Flag(:) = 0 ! assume all observations passed QC
    end if
...
end subroutine opsinputs_fill_fillelementtypefromsimulatedvariable2dobsvalue
