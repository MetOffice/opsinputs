...
SUBROUTINE gc_brecv (msg, len1, send, istat, rarr, sarr)
!     ******************************************************************
!     * Purpose:
!     *
...
IF (send  ==  me) THEN  ! Receiving from myself
  llen=len1/gc__isize
  IF (MOD(len1, INT(gc__isize,gc_int_kind))  /=  0) llen = llen + 1
  DO i=1,llen
    rarr(i)=sarr(i)
  END DO
ELSE ! Receiving from another processor
  IF (send  ==  gc__any) THEN
    CALL mpl_recv(rarr, len1, mpl_byte, mpl_any_source, msg,                   &
         gc__my_mpi_comm_world, STAT, istat)
  ELSE
    CALL mpl_recv(rarr, len1, mpl_byte, send, msg,                             &
         gc__my_mpi_comm_world,STAT, istat)
  END IF
END IF ! IF (send  ==  me)
#endif
